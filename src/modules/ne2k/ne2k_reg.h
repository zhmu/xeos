...
#define NE2K_CR_PAGE_1	(NE2K_CR_PS0)
#define NE2K_CR_PAGE_2	(NE2K_CR_PS1)
#define	NE2K_CR_PAGE_3	(NE2K_CR_PS1|NE2K_CR_PS0)
...
struct dp8390_ring	{
	...
	uint8_t	next_packet;	/* pointer to next packet */
	uint16_t	count;		/* bytes in packet (length + 4) */
};

/* Some drivers prefer to use byte-constants to get at this structure.  */
#define NE2K_RING_RSR		0	/* receiver status */
#define NE2K_RING_NEXT_PACKET	1	/* pointer to next packet */
#define NE2K_RING_COUNT		2	/* bytes in packet (length + 4) */
#define NE2K_RING_HDRSZ		4	/* Header size */

/*
 * Common constants
 */
#define NE2K_PAGE_SIZE		256	/* Size of RAM pages in bytes */
#define	NE2K_PAGE_MASK		255
